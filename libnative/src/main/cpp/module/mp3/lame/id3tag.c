...
typedef enum NumericStringIDs { ID_DATE = FRAME_ID('T', 'D', 'A', 'T') 
        , ID_TIME = FRAME_ID('T', 'I', 'M', 'E') 
        , ID_TPOS = FRAME_ID('T', 'P', 'O', 'S') 
        , ID_TRACK = FRAME_ID('T', 'R', 'C', 'K') 
        , ID_YEAR = FRAME_ID('T', 'Y', 'E', 'R') 
} NumericStringIDs;
...
typedef enum MiscIDs { ID_TXXX = FRAME_ID('T', 'X', 'X', 'X')
        ...
        , ID_USER = FRAME_ID('U', 'S', 'E', 'R') 
        , ID_PCST = FRAME_ID('P', 'C', 'S', 'T') 
        , ID_WFED = FRAME_ID('W', 'F', 'E', 'D') 
} MiscIDs;
...
id3tag_set_textinfo_utf16(lame_t gfp, char const *id, unsigned short const *text)
{
    ...
    if (frame_id == ID_WFED) {
        return id3v2_add_ucs2(gfp, frame_id, 0, text, 0); 
    }
...
id3tag_set_textinfo_latin1(lame_t gfp, char const *id, char const *text)
{
    ...
    if (frame_id == ID_WFED) {
        return id3v2_add_latin1(gfp, frame_id, 0, text, 0); 
    }
    if (isFrameIdMatching(frame_id, FRAME_ID('T', 0, 0, 0))
      ||isFrameIdMatching(frame_id, FRAME_ID('W', 0, 0, 0))) {
        return id3v2_add_latin1(gfp, frame_id, 0, 0, text);
