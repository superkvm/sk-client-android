...
lame_init_params(lame_global_flags * gfp)
{
    ...
    if (gfp->VBR == vbr_off && gfp->compression_ratio > 0) {
        ...
        if (gfp->samplerate_out == 0)
            gfp->samplerate_out = map2MP3Frequency((int) (0.97 * gfp->samplerate_in)); 
    ...
    cfg->mode_gr = cfg->samplerate_out <= 24000 ? 1 : 2; 
...
save_gain_values(lame_internal_flags * gfc)
{
    ...
    if (cfg->findReplayGain) {
        ...
        if (NEQ(RadioGain, GAIN_NOT_ENOUGH_SAMPLES)) {
            rov->RadioGain = (int) floor(RadioGain * 10.0 + 0.5); 
        }
        else {
            rov->RadioGain = 0;
        }
    ...
    if (cfg->findPeakSample) {
        rov->noclipGainChange = (int) ceil(log10(rov->PeakSample / 32767.0) * 20.0 * 10.0); 

        if (rov->noclipGainChange > 0) { 
            rov->noclipScale = floor((32767.0f / rov->PeakSample) * 100.0f) / 100.0f; 
        }
        else            
