...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode, float bandwidth) {
	...
	int result = 0;
	const bool geo_changed = (requestWidth != width) || (requestHeight != height) || (requestMode != mode);
	const bool fps_changed = (requestMinFps != min_fps) || (requestMaxFps != max_fps);
	const bool bw_changed = (requestBandwidth != bandwidth);
	if (geo_changed || fps_changed || bw_changed) {
		requestWidth = width;
			...
			for ( ; LIKELY(isRunning()) ; ) {
				...
				if (LIKELY(frame_mjpeg)) {
					...
					if (LIKELY(!result)) {
						...
						if (mCaptureWindow || mFrameCallbackObj) {
