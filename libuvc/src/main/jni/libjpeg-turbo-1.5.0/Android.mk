...
# Add -fPIC for NDK r29 compatibility (required when static library is linked into shared library)
LOCAL_CFLAGS += -fPIC

#リンクするライブラリを(モジュールにするは)
#LOCAL_LDLIBS := -L$(SYSROOT)/usr/lib -ldl	# to avoid NDK issue(no need for static library)

#このモジュールをモジュールとしてリンクするのライブラリを
...
# Note: yasm doesn't support -fPIC flag, PIC is handled by the assembler code itself
# LOCAL_ASMFLAGS += -fPIC
